...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden font-sans">
      {/* ═══════════════════════════════════════════════════════════
          NAVBAR — Masking top of video with sleek TransitOps executive bar
      ═══════════════════════════════════════════════════════════ */}
      <header className="absolute top-0 left-0 right-0 z-50 h-24 bg-[#0B0F19] border-b border-white/15 px-6 md:px-12 flex items-center justify-between shadow-2xl">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-3.5">
          <div className="w-10 h-10 rounded-xl bg-[#FF5A36] text-white flex items-center justify-center shadow-lg shadow-[#FF5A36]/25">
            <Truck className="w-5 h-5" />
          </div>
          <div className="flex flex-col">
            <span className="font-extrabold text-xl text-white tracking-tight leading-none">TransitOps</span>
            <span className="text-[10px] font-bold tracking-widest text-[#FF5A36] uppercase mt-1">Enterprise OS</span>
          </div>
        </Link>

        {/* Nav Links */}
        <nav className="hidden lg:flex items-center gap-8 text-sm font-semibold text-white/85">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/signin"
            className="px-5 py-2.5 rounded-xl border border-white/20 bg-white/5 hover:bg-white/15 text-white font-semibold text-sm transition-all"
          >
            Sign In
          </Link>
          <Link
            href="/dashboard"
            className="px-6 py-2.5 rounded-xl bg-[#FF5A36] hover:bg-[#E04826] text-white font-bold text-sm shadow-xl shadow-[#FF5A36]/30 transition-all flex items-center gap-2"
          >
      ...
      <section className="relative w-full h-screen min-h-[640px] flex flex-col justify-end overflow-hidden bg-[#0A0E1A]">
        ...
        {/* Top Mask layers to ensure original video navbar text is completely hidden */}
        <div className="absolute top-0 left-0 right-0 h-24 bg-[#0B0F19] z-20 pointer-events-none" />
        <div className="absolute top-24 left-0 right-0 h-16 bg-gradient-to-b from-[#0B0F19] to-transparent z-20 pointer-events-none" />
